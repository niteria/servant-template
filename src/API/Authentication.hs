...
data AuthenticationAPI mode = AuthenticationAPI
  ...
  { register :: mode :- "register" :> ReqBody '[ JSON] Credentials :> Post
      '[ JSON]
      (Id User)
    -- | Given some 'Login' data, generates an authentication token
  , login :: mode :- "login" :> ReqBody '[ JSON] Credentials :> Post
      '[ JSON]
      Token
...
registerEndpoint ::
  ...
  -> Credentials
  -> Handler (Id User)
registerEndpoint passwordManager userRepository login'
  -- hash the password
 = do
...
loginEndpoint ::
  ...
  -> Credentials
  -> Handler Token
loginEndpoint passwordManager authHandler login'
