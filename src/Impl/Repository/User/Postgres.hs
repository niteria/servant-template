module Impl.Repository.User.Postgres
  ( repository
  ) where
...
repository handle =
  UserRepository
    {findByName = postgresGetUserByName handle, add = postgresAddUser handle}
...
postgresAddUser handle name password
  -- Generate the UUID for the user
 = do
  userId' <- liftIO nextRandom
  let query =
        Query.addUser . litUser $
        serializeUser (Id userId') (User name password)
...
runRepositoryQuery handle =
  withExceptT liftRepositoryError . ExceptT . DB.runQuery handle
