...
tagSchema =
  TableSchema
    { name = "tags"
    , schema = Nothing
    , columns = Tag {tagId = "id", tagName = "name"}
    }
...
litContent (Content id' content' userId') =
  Content (lit id') (lit content') (lit userId')
...
contentsTagsSchema =
  TableSchema
    { name = "contents_tags"
    , schema = Nothing
    , columns = ContentsTags {ctContentId = "content_id", ctTagId = "tag_id"}
    }
...
userSchema =
  TableSchema
    ...
    , schema = Nothing
    , columns =
        User {userId = "id", userName = "name", userPassword = "password"}
    }
