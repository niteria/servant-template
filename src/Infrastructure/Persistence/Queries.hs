...
contentsTagsForContent content =
  ...
  filter (\contentTag' -> ctContentId contentTag' ==. contentId content)
...
userForContent content =
  each userSchema >>= filter (\user -> userId user ==. contentUserId content)
...
selectUserContents userId' =
  statement () . select $
  -- Select all content for the given user
   do
    content <-
      each contentSchema >>=
      filter (\content -> contentUserId content ==. lit userId')
...
contentTag content tag =
  ContentsTags {ctContentId = contentId content, ctTagId = tagId tag}
...
addContentWithTags content tags =
  transaction Serializable Write $ do
    ...
    Transaction.statement () $
      add
        contentsTagsSchema
        (contentTag (litContent content) <$>
         (litTag <$> alreadyPresentTags) <> newTags)
