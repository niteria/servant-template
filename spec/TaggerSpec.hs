...
import Tagger.Authentication.Credentials
...
registerUser env login' =
  runClientM ((register . authentication $ apiClient) login') env
...
addUserContent ::
  ...
  -> Token
  -> Content Tag
  -> IO (Either ClientError (Id (Content Tag)))
addUserContent env token content =
  runClientM
    ((addContent . tagger apiClient) (toServantToken token) content)
    env
...
getUserContents ::
  ...
  -> Token
  -> [Tag]
  -> IO (Either ClientError [Owned (Content Tag)])
getUserContents env token tags =
  runClientM ((getContents . tagger apiClient) (toServantToken token) tags) env

spec :: Spec
spec =
  around withTaggerApp $ do
    ...
    describe "Tagger" $ do
      describe "register user" $ do
        it "should register a user" $ \port -> do
          response <-
            registerUser
              (clientEnv port)
              (Credentials "marcosh" (Password "password"))
        ...
        it "should not register two users with the same name" $ \port -> do
          ...
          response <-
            registerUser
              (clientEnv port)
              (Credentials "marcosh" (Password "password1"))
        ...
        it "should register two users with different names" $ \port -> do
          ...
          response <-
            registerUser
              (clientEnv port)
              (Credentials "perons" (Password "password"))
          response `shouldSatisfy` isRight
      describe "login" $ do
      ...
      describe "addContent" $ do
        it "allows a user to add a new content" $ \port -> do
          ...
          let content =
                createContent "some content" [Tag "first tag", Tag "second tag"]
      ...
      describe "getContents" $ do
        it "retrieves all contents added by a user" $ \port -> do
          ...
          let content1 =
                createContent "some content" [Tag "first tag", Tag "second tag"]
          let content2 =
                createContent "other content" [Tag "first tag", Tag "third tag"]
          ...
          case contents of
            ...
            Right ownedContent ->
              ownedContent `shouldMatchList`
              [Owned userId content1, Owned userId content2]
        ...
        it "retrieves only contents with a given tag" $ \port -> do
          ...
          case contents of
            Left _ -> fail "unable to retrieve contents"
            Right ownedContent ->
              ownedContent `shouldMatchList` [Owned userId content1]
        ...
        it "retrieves only contents from the requesting user" $ \port -> do
          ...
          case contents of
            ...
            Right ownedContent ->
              ownedContent `shouldMatchList`
              [Owned userId1 content1, Owned userId1 content2]
